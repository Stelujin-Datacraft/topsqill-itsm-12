-- Harden signup bootstrap for environments where the previous RPC was not applied
-- or org RLS was toggled manually. Domain/description are DB columns only —
-- they are never collected from the signup form (description stays null).

-- Allow authenticated users to create an organization when they are its admin_email
...
-- Keep / recreate secure RPC used by signup
...
BEGIN
  ...
  -- Domain is a required DB column; never collected from the UI.
  ...
  INSERT INTO public.organizations (name, domain, description, admin_email, status)
  VALUES (v_name, v_domain, NULL, v_email, 'active')
  ...
  INSERT INTO public.projects (name, description, organization_id, created_by, status)
  VALUES ('Default Project', NULL, v_org_id, v_user_id, 'active')

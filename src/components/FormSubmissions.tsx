...
export function FormSubmissions({ form }: FormSubmissionsProps) {
  ...
  const filteredSubmissions = submissions.filter(submission => {
    const matchesSearch = submission.submittedBy.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         Object.values(submission.submissionData).some(value => 
                           String(value).toLowerCase().includes(searchTerm.toLowerCase())
                         );
    
    const matchesApprovalFilter = approvalFilter === 'all' || 
                                 submission.approvalStatus === approvalFilter;
  ...
  const handleExportData = () => {
    ...
    const headers = [
      ...
      'Approval Date',
      'Approval Notes',
      ...allFields.map(field => field.label)
    ];
    const csvData = [
      headers.join(','),
      ...filteredSubmissions.map(submission => [
        ...
        ...allFields.map(field => submission.submissionData[field.id] || '')
      ].join(','))
    ].join('\n');
          ...
          <CardContent>
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                {selectedSubmission.approvalTimestamp && (
                  <div>

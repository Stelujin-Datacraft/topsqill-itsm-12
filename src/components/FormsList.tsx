  ...
  const renderGridView = () => (
    <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
      {visibleForms.map((form) => {
        ...
        return (
          <Card key={form.id} className="hover:shadow-lg transition-all duration-300 hover:scale-[1.02]">
            <CardHeader>
              <div className="flex items-start justify-between">
                ...
                <div className="flex space-x-1">
                  ...
                  <Button
                    variant="ghost"
                    size="sm"
                    onClick={() => handleFormShare(form.id)}
                    title="Share Form"
  ...
  const renderListView = () => (
    <div className="space-y-4">
      {visibleForms.map((form) => {
        ...
        return (
          <Card key={form.id} className="hover:shadow-lg transition-all duration-300">
    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {statusColumns.map((status) => (
          <div key={status.key} className={`space-y-4 p-4 rounded-lg border-2 ${status.bgColor} min-h-[400px]`}>
            ...
            <div className="space-y-3">
              {visibleForms.filter(f => f.status === status.key).map((form) => {
                ...
                return (
                  <Card key={form.id} className="hover:shadow-md transition-all duration-200 hover:scale-[1.02] bg-white">
                    <CardContent className="p-4">
                      <div className="space-y-3">
                        <div className="flex items-start justify-between">
                          ...
                          <Badge 
                            variant={form.isPublic ? 'default' : 'secondary'}
                            className={`ml-2 shrink-0 ${form.isPublic 
                              ? 'bg-cyan-500 hover:bg-cyan-600 text-white border-cyan-400' 
                              : 'bg-slate-600 hover:bg-slate-700 text-white border-slate-500'
                            }`}
                          >

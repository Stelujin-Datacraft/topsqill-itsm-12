...
const integrations = [
  ...
  { name: "Microsoft Teams", icon: Users, color: "bg-info/10 text-info", category: "Notifications" },
  { name: "Email (SMTP)", icon: Mail, color: "bg-red-100 text-red-700", category: "Notifications" },
  { name: "SendGrid", icon: Send, color: "bg-info/10 text-info", category: "Notifications" },
  ...
  { name: "Dropbox", icon: FolderOpen, color: "bg-info/10 text-info", category: "Storage" },
  { name: "OneDrive", icon: FolderOpen, color: "bg-sky-100 text-sky-700", category: "Storage" },
  { name: "PostgreSQL", icon: Database, color: "bg-info/10 text-info", category: "Database" },
  { name: "Supabase", icon: Database, color: "bg-success/10 text-success", category: "Database" },
    ...
    <section id="integrations" className="py-20 bg-muted/20">
      <div className="container mx-auto px-4">
        <div className="text-center mb-12">
          <Badge variant="secondary" className="mb-4 bg-info/10 text-info">
            Integrations
          </Badge>
        ...
        <div className="flex flex-wrap justify-center gap-3 max-w-4xl mx-auto">
          {integrations.map((integration, index) => (
            <div
              key={index}
              className="group flex items-center gap-2 px-4 py-2.5 rounded-full bg-background border border-border/50 hover:border-primary/30 hover:shadow-md transition-all duration-300 cursor-pointer"
            >
              <div className={`p-1.5 rounded-full ${integration.color} group- transition-transform duration-300`}>
                <integration.icon className="icon-md" />
              </div>

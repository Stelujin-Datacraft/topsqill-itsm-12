...
interface EnhancedWorkflowAccessManagerProps {
  workflowId: string;
  workflowName: string;
}

export function EnhancedWorkflowAccessManager({ workflowId, workflowName }: EnhancedWorkflowAccessManagerProps) {
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground">
            Manage access to "{workflowName}" - permissions are determined by user roles
          </p>
        </CardHeader>
...
function WorkflowUserAccessRow({ 
  user, 
  workflowId, 
  expanded, 
  onToggleExpansion, 
  ...
  const permissions = getUserPermissions('workflows', workflowId);
  const topLevel = topLevelPermissions.workflows;
    ...
    <div className="border rounded-lg">
      ...
      {expanded && (
        <div className="border-t bg-gray-50 p-4">
          <div className="space-y-3">
            <div className="text-sm font-medium">Workflow Permissions</div>
            
            {/* Top-level permission info */}
            <div className="text-xs text-muted-foreground bg-white p-2 rounded">
              <strong>Top-level Workflow Access:</strong> 
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                { id: 'view', label: 'View Workflow', checked: permissions.view },
                { id: 'edit', label: 'Edit Workflow', checked: permissions.edit },
                { id: 'create', label: 'Create Workflow', checked: permissions.create },
                { id: 'delete', label: 'Delete Workflow', checked: permissions.delete }
              ].map((perm) => (

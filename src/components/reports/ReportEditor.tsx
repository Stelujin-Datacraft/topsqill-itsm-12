...
export function ReportEditor({ reportId, reportName, onSave }: ReportEditorProps) {
  ...
  const handleDrilldown = (componentId: string, drilldownLevel: string, drilldownValue: string) => {
    ...
    setDrilldownStates(prev => {
      ...
      if (!config?.drilldownConfig?.levels) return prev;
      ...
      if (currentLevel < config.drilldownConfig.levels.length) {
      ...
      return {
        ...prev,
        [componentId]: {
          path: config.drilldownConfig.levels.slice(0, newValues.length),
          values: newValues
        }
      };
    });

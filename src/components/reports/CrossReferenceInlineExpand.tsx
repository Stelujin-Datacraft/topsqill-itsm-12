...
import { Loader2, ExternalLink, Link2, Plus, Minus, ChevronRight } from 'lucide-react';
...
export function CrossReferenceInlineExpand({
        ...
        const details = submissions.map((sub) => {
          ...
          for (const field of formFields) {
            ...
            if (field.field_type === 'cross-reference' || field.field_type === 'child-cross-reference') {
              ...
              regularFields.push({
                id: field.id, label: field.label, fieldType: field.field_type,
                value, options: field.options, customConfig,
              });
            }
          }
  ...
  if (loading) {
    return (
      <div className="flex items-center gap-2 py-3 px-3 mt-1 border border-border rounded-md bg-background shadow-lg relative z-50 min-w-[500px]">
  ...
  return (
    <div className="mt-1 space-y-2 relative z-50 min-w-[600px] w-full max-w-[900px]">
      {recordDetails.map((rec) => (
        <div key={rec.id} className="border border-border rounded-md overflow-hidden bg-background shadow-lg">
          <table className="w-full text-xs">
            <thead>
              <tr className="bg-muted border-b border-border">
                <th className="px-3 py-2 text-left font-semibold text-muted-foreground w-[35%]">Field</th>
            ...
            <tbody>
              ...
              <tr className="bg-accent/10 border-b border-border/40">
                <td className="px-3 py-2 font-semibold text-foreground whitespace-nowrap">ID</td>
              ...
              {rec.fields
                ...
                .map((field, idx) => (
                  <tr key={field.id} className={idx % 2 === 0 ? 'bg-background' : 'bg-muted/30'}>
                    <td className="px-3 py-2 font-medium text-muted-foreground whitespace-nowrap align-top">
                      {field.label}
                    </td>
                    <td className="px-3 py-2 break-words" title={formatValue(field)}>
                      {formatValue(field)}
                    </td>
                  </tr>
                ))}
              ...
              {rec.crossRefFields.map((cr) => (
                <div key={cr.fieldId} className="space-y-1">
                  ...
                  {cr.linkedRecords.map((linked: any) => {
                    ...
                    return (
                      <div key={linked.id} className="rounded-md border border-border bg-background">
      ...
      try {
        ...
        for (const field of formFields) {
          ...
          if (field.field_type === 'cross-reference' || field.field_type === 'child-cross-reference') {
            ...
            regularFields.push({ id: field.id, label: field.label, fieldType: field.field_type, value, options: field.options, customConfig });
          }
        }
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="bg-muted/60 border-b border-border/30">
              <th className="px-3 py-1.5 text-left font-semibold text-muted-foreground w-[35%]">Field</th>
          ...
          <tbody>
            {visibleFields.map((field, idx) => (
              <tr key={field.id} className={idx % 2 === 0 ? 'bg-background/30' : 'bg-muted/10'}>
                <td className="px-3 py-1.5 font-medium text-muted-foreground whitespace-nowrap align-top">{field.label}</td>
                <td className="px-3 py-1.5 break-words" title={formatValue(field)}>{formatValue(field)}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
function formatValue(field: FieldDisplay): string {
  ...
  if ((fieldType === 'select' || fieldType === 'radio' || fieldType === 'checkbox' || fieldType === 'dropdown') && options) {
    ...
    if (Array.isArray(value)) {
      return value.map((v) => { const opt = opts.find((o: any) => o.value === v || o.id === v); return opt?.label || v; }).join(', ');
    }

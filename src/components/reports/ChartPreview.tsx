...
export function ChartPreview({
  ...
  const processCrossReferenceData = async (
    ...
    try {
      ...
      // drilldownValues[0] = parentRefId (clicked parent's ID)
      // drilldownValues[1] = drilldownLevels[0] field value
      // drilldownValues[2] = drilldownLevels[1] field value
      // currentFieldLevel is offset by 1 to account for parentRefId at position 0
      const currentFieldLevel = drilldownValues.length > 0 ? drilldownValues.length - 1 : -1;
      // When currentFieldLevel = -1 (no drilling), show parent-level data (normal cross-ref result)
      // When currentFieldLevel = 0, show data grouped by drilldownLevels[0]
      // When currentFieldLevel = 1, show data grouped by drilldownLevels[1]
      const currentDimensionField = currentFieldLevel >= 0 ? drilldownLevels[currentFieldLevel] : undefined;
      ...
      // DRILLDOWN GROUPING: Only group by drilldown field AFTER user has clicked on a parent
      // hasStartedDrilling means drilldownValues has at least the parentRefId
      ...
      // Show drilldown-grouped data only AFTER user has clicked on a parent (hasStartedDrilling)
      // AND when there's a valid dimension field to group by (currentFieldLevel within bounds)
  ...
  const renderChart = () => {
    ...
    const preserveAsStringKeys = ['name', '_drilldownData', 'xRaw', 'yRaw', 'xOriginal', 'yOriginal', 'xFieldName', 'yFieldName', 'xFieldLabel', 'yFieldLabel', 'submissionId', '_legendMapping', '_xLegendMapping', '_yLegendMapping', 'rawSecondaryValue', 'rawYValue', '_isCompareEncoded', '_isCrossRefCompare', '_hasTextX', '_hasTextY', '_xIsText', '_yIsText', 'parentId', 'parentRefId', 'linkedSubmissionId', '_linkedSubmissionIds', '_allParentIds', '_allParentRefIds', '_optionColor', '_optionImage', '_xOptionColor', '_yOptionColor', '_xOptionImage', '_yOptionImage', '_drilldownField', '_drilldownValue', 'rawValue'];
              ...
              <ResponsiveContainer width="100%" height="100%">
                <RechartsLineChart data={sortedData} margin={{ top: 20, right: 30, left: 60, bottom: 60 }}>
                  ...
                  <YAxis 
                    ...
                    tick={{ fontSize: 11 }}
                    domain={yDomain}
                    label={{ value: field2Name, angle: 0, position: 'insideTopLeft', offset: 10, dy: -10, dx: 5, style: { textAnchor: 'start', fontWeight: 500, fill: 'hsl(var(--foreground))' } }}
                  />
                  {compareTooltip}
          ...
          return (
            <RechartsLineChart data={sanitizedChartData} margin={{ top: 20, right: 20, left: 80, bottom: 80 }}>
              ...
              <YAxis 
                ...
                allowDecimals={false}
                label={{ value: yAxisFieldName, angle: 0, position: 'insideTopLeft', offset: 10, dy: -10, dx: 5, style: { textAnchor: 'start', fontWeight: 500, fill: 'hsl(var(--foreground))' } }}
              />
              <Tooltip content={({ payload }) => {
                if (!payload || payload.length === 0) return null;
                const data = payload[0]?.payload;
              ...
              <ResponsiveContainer width="100%" height="100%">
                <RechartsScatterChart margin={{ top: 20, right: 30, left: 80, bottom: 80 }}>
                    ...
                    <YAxis 
                      ...
                      label={{ value: bubbleYLabel, angle: 0, position: 'insideTopLeft', offset: 10, dy: -10, dx: 5, style: { textAnchor: 'start', fontWeight: 500, fill: 'hsl(var(--foreground))' } }}
        ...
        {drilldownState?.values && drilldownState.values.length > 0 && <div className="flex items-center gap-2 p-2 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-700 mt-3">
            ...
            <div className="flex items-center gap-1 flex-wrap">
              {drilldownState.values.map((value, index) => {
                ...
                // drilldownLevels[index-1] because drilldownValues[0] is parentRefId
                let fieldName: string;
                if (isCrossRef) {
                  if (index === 0) {
                    fieldName = 'Parent Record';
                  } else {
                    fieldName = getFormFieldName(drilldownLevels[index - 1] || '');
                  }
                } else {
                  fieldName = getFormFieldName(drilldownLevels[index] || '');
                }
              ...
              <div className="flex flex-col gap-3">
                ...
                {drilldownState?.values?.length > 0 && (
                  <div className="flex justify-end">
                    <Button size="sm" variant="outline" onClick={resetDrilldown}>
                      Reset
                    </Button>
                  </div>
                )}

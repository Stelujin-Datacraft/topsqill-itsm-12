...
export function SavedQueriesSection({ savedQueries, isLoading, onSelectQuery, onDeleteQuery }: SavedQueriesSectionProps) {
  ...
  return (
    <div className="border-b border-border">
      ...
      >
        {isExpanded ? <ChevronDown className="h-4 w-4 text-module-overview" /> : <ChevronRight className="h-4 w-4 text-module-overview" />}
          ...
          {isLoading ? (
            ...
            savedQueries.map((query) => (
              <div key={query.id} className="flex items-center group px-3 py-1">
                ...
                >
                  <FileText className="h-3 w-3 mr-2 text-module-forms" />

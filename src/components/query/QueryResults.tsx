...
export const QueryResults: React.FC<QueryResultsProps> = ({
  ...
  return <div className="space-y-4 px-[10px]">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-semibold">Query Results</h3>
      ...
      <div className="border rounded-md">
        <Table>
          <TableHeader>
            <TableRow>
              {columns.map(column => <TableHead key={column} className="font-medium">
          ...
          <TableBody>
            {data.map((row, index) => <TableRow key={index}>
                {columns.map(column => <TableCell key={column} className="max-w-xs truncate">
      ...
      {totalPages > 1 && onPageChange && <div className="flex justify-center">
          <Pagination>
            <PaginationContent>
              <PaginationItem>
            ...
            return <PaginationItem key={pageNum}>
                    <PaginationLink onClick={() => onPageChange(pageNum)} isActive={currentPage === pageNum} className="cursor-pointer">
                      {pageNum}
                    </PaginationLink>
                  </PaginationItem>;
          })}

  ...
  static async executeCreateRecordAction(context: NodeExecutionContext): Promise<ActionExecutionResult> {
    ...
    try {
      ...
      for (let i = 0; i < recordCount; i++) {
        ...
        console.log(`📝 Prepared record ${i + 1}/${recordCount} with data:`, submissionData);
        rowsToInsert.push({
      ...
      for (let offset = 0; offset < rowsToInsert.length; offset += INSERT_BATCH) {
        ...
        if (insertError) {
          console.error(`❌ Error creating records batch at offset ${offset}:`, insertError);
          return {
            success: false,
            error: `Failed to create records: ${insertError.message}`,
            actionDetails: {
              ...actionDetails,
        ...
        for (const newSubmission of insertedRows || []) {
          createdRecords.push(newSubmission.id);
          console.log(`✅ Created record: ${newSubmission.id}`);

          try {
            const submissionData = (newSubmission.submission_data || {}) as Record<string, unknown>;
            const fieldLabels: Record<string, string> = {};
            ...
            const changes = Object.entries(submissionData).map(([fieldId, value]) => ({
              ...
              oldValue: null,
              newValue: value !== null && value !== undefined
  ...
  static async executeCreateCombinationRecordsAction(context: NodeExecutionContext): Promise<ActionExecutionResult> {
    ...
    try {
      ...
      if (firstSourceRecords.length === 0) {
        const raw = triggerSubmissionData[config.sourceCrossRefFieldId];
        console.warn('⚠️ No linked records in first source XR. Raw value:', raw);
        return {
          success: true,
          output: {
            createdCount: 0,
            ...
            sourceCrossRefFieldId: config.sourceCrossRefFieldId,
            sourceLinkedFormId: config.sourceLinkedFormId,
            rawCrossRefValue: raw ?? null,
          },
          actionDetails
        };
      }
      ...
      if (combinationMode === 'dual') {
        ...
        if (secondSourceRecords.length === 0) {
          const raw = triggerSubmissionData[config.secondSourceCrossRefFieldId];
          console.warn('⚠️ No linked records in second source XR. Raw value:', raw);
          return {
            success: true,
            output: {
              createdCount: 0,
              ...
              secondSourceCrossRefFieldId: config.secondSourceCrossRefFieldId,
              secondSourceLinkedFormId: config.secondSourceLinkedFormId,
              rawCrossRefValue: raw ?? null,
            },
            actionDetails
          };

...
export type ExecuteFeedOptions = {
  notifyFailure?: (body: Record<string, unknown>) => Promise<unknown>;
};
...
async function loadExternalRecords(
  ...
  if (sourceType === 'database') {
    ...
    const client = new PgClient({ connectionString: cfg.connectionString });
    let rows: any[] = [];
    try {
...
export async function executeDataFeed(supabase: SupabaseClient, params: Record<string, unknown>, options?: ExecuteFeedOptions) {
  ...
  try {
    ...
    if (!feedId) {
      return { success: false, error: 'feedId is required' };
    }
    ...
    if (feedError || !feed) {
      ...
      return { success: false, error: 'Data feed not found' };
    }
